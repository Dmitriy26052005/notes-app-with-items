...
fun deleteAnItem() {
    ...
    if (note != null) {
        ...
        if (item != null) {
            ...
            if (isDeleted) {
                println("Delete Succseeful!")
            } else {
                println("De;ete not Successful")
            }
        }
    }
}
...
fun markItemStatus() {
    val note: Note? = askUserToChooseActiveNote()
    if (note != null) {
        var item: Item? = askUserToChooseItem(note)
        if (item != null) {
            var changeStatus = 'X'
            if (item.isItemComplete) {
                changeStatus = readNextChar("The item is currently complete...do you want to mark it as TOD0?")
...
fun listToDoItems() {
    if (noteAPI.numberOfItemsToDO() > 0) {
